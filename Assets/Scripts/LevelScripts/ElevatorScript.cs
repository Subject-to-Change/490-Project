...
{ private int activated = 0;
    ...
    startPosition = GameObject.Find("ElevatorBase").transform.position;
     
    //grab position of destination 
    endPosition = GameObject.Find("Destination").transform.position;
    ...
    void FixedUpdate()
    {
        ...
        if (activated == 1 && destination == 0)
        {
            ...
            if(endPosition.y - transform.position.y < .01)
            {
        ...
        else if (activated == 1 && destination == 1)
        {
            ...
            if (transform.position.y - startPosition.y < .01)
            {
                this.destination = 0;
                velocity = speed;
            }
    ...
    {
        player.transform.SetParent(GameObject.Find("Elevator").transform);
        activated = 1;
